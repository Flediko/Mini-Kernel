...
void get_memory_info(unsigned long *total_kb, unsigned long *used_kb, unsigned long *free_kb)
{
	/* For now, we'll use static values based on what we've allocated */
	*total_kb = TOTAL_MEMORY_KB;
	
	/* Calculate used memory: page directory + page table + kernel (~4MB mapped) */
	*used_kb = (PAGE_SIZE * 2) / 1024 + 4096;  /* Approx 4MB for kernel */
